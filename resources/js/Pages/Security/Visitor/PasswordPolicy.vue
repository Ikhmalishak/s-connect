...
const strengthDescription = computed(() => {
  const requirements = [];
  if (policy.value.min_length > 0) requirements.push(`${policy.value.min_length}+ characters`);
  if (policy.value.require_letters) requirements.push("letters");
  if (policy.value.require_mixed_case) requirements.push("uppercase & lowercase");
...
const securityLevel = computed(() => {
  ...
  if (score >= 5) return { level: "Strong", color: "text-green-600", bg: "bg-green-100" };
  if (score >= 3) return { level: "Medium", color: "text-yellow-600", bg: "bg-yellow-100" };
...
const passwordTestResult = computed(() => {
  ...
  if (policy.value.require_mixed_case && (!/[a-z]/.test(testPassword.value) || !/[A-Z]/.test(testPassword.value))) {
  ...
  if (policy.value.require_symbols && !/[!@#$%^&*(),.?":{}|<>]/.test(testPassword.value)) {
...
async function savePolicy() {
  ...
  try {
    ...
    successMessage.value = res.data.message || "Password policy updated successfully!";
    originalPolicy.value = { ...policy.value };
  } catch (error: any) {
    errorMessage.value = error.response?.data?.message || "Failed to update policy. Please try again.";
  } finally {
    saving.value = false;
  }
}
  ...
  <AdminAuthenticatedLayout>
    ...
    <Card class="shadow-lg p-2 mb-6 bg-gray-100">
      <div class="flex items-center justify-between">
        <div class="flex items-center">
          ...
          <div>
            <h1 class="text-xl font-bold text-gray-800">Password Policy Settings</h1>
            <p class="text-gray-600 mt-1">Configure security requirements for user passwords</p>
          </div>
        </div>
        <div class="text-right">
          <div class="text-sm text-gray-500">Security Level</div>
          <div class="flex items-center mt-1">
            <span class="px-3 py-1 rounded-full text-sm font-semibold" :class="[securityLevel.color, securityLevel.bg]">
              {{ securityLevel.level }}
            </span>
          </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2">
        <Card class="shadow-lg p-6">
          <div class="flex items-center justify-between mb-6">
            <h2 class="text-xl font-semibold text-gray-800">Policy Configuration</h2>
          ...
          <div v-if="!loading" class="space-y-6">
            ...
            <div class="bg-white border border-gray-200 p-4 rounded-lg">
              ...
              <div class="flex items-center gap-4">
                <input
                  ...
                  class="flex-1"
                />
                <div class="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm font-semibold min-w-[60px] text-center">
                  {{ policy.min_length }}
                </div>
              </div>
            ...
            <!-- Requirements -->
            <div class="bg-white border border-gray-200 p-4 rounded-lg">
              <h3 class="text-sm font-medium text-gray-700 mb-4">Character Requirements</h3>
              <div class="space-y-3">
                <label class="flex items-center group cursor-pointer">
                  ...
                  />
                  <div class="ml-3">
                    <span class="text-sm font-medium text-gray-700 group-hover:text-gray-900">Require Letters (a-z, A-Z)</span>
                    <p class="text-xs text-gray-500">Password must contain alphabetic characters</p>
                  </div>
                </label>
                ...
                <label class="flex items-center group cursor-pointer">
                  ...
                  />
                  <div class="ml-3">
                    <span class="text-sm font-medium text-gray-700 group-hover:text-gray-900">Require Mixed Case</span>
                    <p class="text-xs text-gray-500">Password must contain both uppercase and lowercase letters</p>
                  </div>
                </label>
                ...
                <label class="flex items-center group cursor-pointer">
                  ...
                  />
                  <div class="ml-3">
                    <span class="text-sm font-medium text-gray-700 group-hover:text-gray-900">Require Numbers (0-9)</span>
                    <p class="text-xs text-gray-500">Password must contain numeric characters</p>
                  </div>
                </label>
                ...
                <label class="flex items-center group cursor-pointer">
                  ...
                  />
                  <div class="ml-3">
                    <span class="text-sm font-medium text-gray-700 group-hover:text-gray-900">Require Special Characters</span>
                    <p class="text-xs text-gray-500">Password must contain symbols (!@#$%^&* etc.)</p>
                  </div>
                </label>
              </div>
            </div>
      ...
      <div class="space-y-6">
        <!-- Policy Preview -->
        <Card class="shadow-lg p-6">
          <h2 class="text-lg font-semibold text-gray-800 mb-4">Current Policy</h2>
          <div class="space-y-3">
            ...
            <div class="flex items-center">
              <span class="px-2 py-1 rounded-full text-xs font-medium" :class="[securityLevel.color, securityLevel.bg]">
        ...
        <!-- Password Tester -->
        <Card class="shadow-lg p-6">
          <h2 class="text-lg font-semibold text-gray-800 mb-4">Test Password</h2>
          <div class="space-y-3">
            <input
            ...
            <div v-if="passwordTestResult" class="space-y-2">
              <div class="flex items-center">
                <svg v-if="passwordTestResult.valid" class="w-5 h-5 text-green-500 mr-2" fill="currentColor" viewBox="0 0 20 20">
                  <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path>
                </svg>
                <svg v-else class="w-5 h-5 text-red-500 mr-2" fill="currentColor" viewBox="0 0 20 20">
                  <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"></path>
                </svg>
                <span class="text-sm font-medium" :class="passwordTestResult.valid ? 'text-green-700' : 'text-red-700'">
                  {{ passwordTestResult.valid ? 'Valid Password' : 'Invalid Password' }}
                </span>
              </div>
              <ul v-if="!passwordTestResult.valid" class="text-xs text-red-600 space-y-1">
                <li v-for="issue in passwordTestResult.issues" :key="issue" class="flex items-center">
        ...
        <!-- Tips -->
        <Card class="shadow-lg p-6 bg-gradient-to-br from-yellow-50 to-orange-50">
