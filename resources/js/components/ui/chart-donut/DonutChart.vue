...
const props = defineProps({
  ...
  legendPosition: {
    type: String,
    required: false,
    default: 'right', // or 'bottom', 'left', etc.
  },
});
  ...
  <div :class="cn('w-full h-48 flex flex-col items-end', $attrs.class ?? '')">
    ...
    >
      ...
      <VisDonut
        ...
        :central-label="type === 'donut' ? (props.centerLabel ?? valueFormatter(totalValue)) : ''"
        :centralSubLabel="type === 'donut' ? (props.centralSubLabel ?? '') : ''"
        :events="{
    ...
    <div v-if="showLegend" :class="legendClass">
      ...
      >
        <div :class="cn('flex items-center', legendPosition === 'bottom' ? 'mb-1' : 'mr-2')">
          <div
            :class="cn('w-3 h-3 rounded-full mr-2', legendPosition === 'bottom' ? 'mr-1' : 'mr-2')"
        ...
        <div :class="cn('flex flex-col', legendPosition === 'bottom' ? 'items-center' : 'items-end')">
          <span class="text-sm font-semibold">{{ valueFormatter(item.value) }}</span>
        </div>
      </div>
    </div>
</template>
