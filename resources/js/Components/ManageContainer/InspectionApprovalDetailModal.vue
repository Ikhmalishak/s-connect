<template>
        ...
        <Transition name="modal-fade">
            ...
            >
                <Transition name="modal-scale" appear>
                    <div
                        v-if="show"
                        class="bg-white p-6 rounded-lg shadow-[0_4px_20px_rgba(255,255,255,0.6)] w-[95%] max-w-4xl max-h-[90vh] overflow-y-auto"
                    >
                        <div
                        ...
                        <div v-else-if="details" class="space-y-6">
                            ...
                            <!-- Inspection Details -->
                            <div v-if="details.inspection" class="bg-white border border-gray-200 rounded-lg p-4">
                                <h3 class="text-lg font-semibold text-gray-900 mb-4">Inspection Results</h3>

                                <!-- Inspection Summary -->
                                <div class="mb-4 p-3 bg-blue-50 rounded-lg">
                                    <div class="flex justify-between items-center">
                                        <div>
                                            <span class="font-medium text-gray-700">Inspection Status:</span>
                                            <span
                                                class="ml-2 px-2 py-1 text-xs font-medium rounded-full"
                                                :class="{
                                                    'bg-green-100 text-green-800': details.inspection.status === 'passed',
                                                    'bg-red-100 text-red-800': details.inspection.status === 'failed',
                                                    'bg-yellow-100 text-yellow-800': details.inspection.status === 'pending'
                                                }"
                                            >
                                        ...
                                        <div class="text-sm text-gray-600">
                                            <div>Received: {{ formatDate(details.inspection.received_at) }}</div>
                                            <div>Inspected: {{ formatDate(details.inspection.inspected_at) }}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div v-else class="text-center py-8">
                            <p class="text-gray-500">No inspection details available.</p>
                        </div>
    ...
    <Dialog :open="imageModalOpen" @open-change="imageModalOpen = false">
        <DialogContent class="max-w-4xl">
            <DialogHeader>
                <DialogTitle>Inspection Evidence</DialogTitle>
            </DialogHeader>
