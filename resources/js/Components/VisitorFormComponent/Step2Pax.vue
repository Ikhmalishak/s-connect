...
// Sync localPaxCount when parent updates paxCount
watch(
  paxCount,
  (newVal) => {
...
// Emit to parent when localPaxCount changes
function handlePaxCountChange() {
    const newCount = parseInt(localPaxCount.value.toString()) || 0;
    const clampedCount = Math.max(0, Math.min(newCount, cappedMax.value));
    
    if (clampedCount !== paxCount.value) {
        localPaxCount.value = clampedCount;
        emit("update:paxCount", clampedCount);
    }
}

// Handle remove pax button click
function handleRemovePax(index: number) {
    emit("removePax", index);
}
</script>

<template>
  <div class="mb-4">
    <label for="pax-count" class="block text-sm font-medium leading-6 text-gray-900">
      How many pax?
    </label>
    <Input
      id="pax-count"
      ...
      @input="handlePaxCountChange"
      @blur="handlePaxCountChange"
      min="0"
      :max="cappedMax"
      class="w-24"
    />
  ...
  <div class="grid grid-cols-3 gap-4">
    ...
    >
      ...
      <FormField :name="`pax[${index}].id_number`" v-slot="{ componentField }">
        <FormItem>
          <FormLabel>
            {{ (field.value as any).id_type === "Passport" ? "Passport Number" : "IC Number" }}
          </FormLabel>
          <FormControl>
      ...
      <FormField :name="`pax[${index}].phone_number`" v-slot="{ componentField }">
        <FormItem>
          ...
          <FormControl>
            <Input type="text" v-bind="componentField" placeholder="Enter Phone Number" />
          </FormControl>
          <FormMessage />
        </FormItem>
      </FormField>
